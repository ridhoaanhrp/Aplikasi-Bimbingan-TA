...
import 'package:bimbingan_ta_app/Screens/Register%20Screen/student_register.dart';
...
class StudentLogin extends StatefulWidget {
  ...
  @override
  State<StudentLogin> createState() => _LoginMahasiswaState();
}

class _LoginMahasiswaState extends State<StudentLogin> {
  ...
  login() async {
    var response = await http
        .post(Uri.parse('http://10.0.2.2:1337/api/students/login'), body: {
    ...
    if (response.statusCode == 200) {
      ...
      await preferences.setString("student_id", data['student_id']);
      await preferences.setInt("id", data['id']);
      await preferences.setString("name", data['name']);
      await preferences.setBool("isStudent", true);
      ...
      Alert(
        ...
        style: const AlertStyle(
            ...
            titleStyle: TextStyle(
                ...
                fontWeight: FontWeight.w800)),
        title: "Login Gagal",
        desc: "NIM atau Password mungkin salah",
        buttons: [
          DialogButton(
              color: const Color(0xFFE31C21),
              child: const Text(
